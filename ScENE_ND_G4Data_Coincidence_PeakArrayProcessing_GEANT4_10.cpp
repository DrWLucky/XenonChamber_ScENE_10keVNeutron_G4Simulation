...
April 8th 2013
 ...
 1. All the collected data is from either Active LAr volume or the Neutron counter
 EJ301. So the hit identification simply uses the distance.
 
 2. Hits in different EJ301 are used to create separate peaks to eliminiate the difference.
...
//void test()
{
    ...
    Int_t           iDetector1_MultipleScattering = -1;
...
//----------------------------------------------------------------------
    ...
    Int_t         Detector1_MultipleScattering = -1;
    ...
    for (Int_t k=0; k<TotG4RawEventNbr; k++)
{
    ...
    Detector1_MultipleScattering = -1;
    Detector1_PeakNbr = 0;
    ...
    for(int i=0; i<TotStepNbr; i++)
    {
            ...
            //if( (PositionY->at(i)<Detector1_MaxPositionY)&&(PositionY->at(i)>Detector1_MinPositionY)&&(PositionX->at(i)<Detector1_MaxPositionX)&&(PositionX->at(i)>Detector1_MinPositionX)&&(globalTime->at(i)<DecayChainTimeLimit) ) //-------- To check whether in LAr active volume ----------
        
        std::string tempPhysicalVolume =  PhysicalVolume->at(i); //---- name of physical volume -----
        
        
        if(tempPhysicalVolume==std::string("ActiveLAr Volume Physical")) //---- Using physical volume name to Identify the Volume -----
            {
                
                
                //---Count the multiple scattering---
                std::string tempCreatorProcess =  CreatorProcess->at(i);

                
                //---- the name of interaction is changed to hadElastic, in GEANT4.10 and versions later ----
                if( (tempCreatorProcess==std::string("hadElastic"))&&(dDepositeEnergy->at(i)>0) ){
                    ...
                    bool PeakTrigger = false;
                        ...
                        for(unsigned int k=0; k<fDetector1_TotPeakEnergy->size(); k++)
                        {
                                    ...
                                    if( (fDetector1_TotPeakEnergy->at(k)>Detector1_TriggerThreshold)&&(PeakTrigger==false) )
                                    {
                                            ...
                                            //Detector1_TriggerTime         = fDetector1_MaxPeakTime->at(k);
